...
public class Serv:MonoBehaviour
{
    ...
    //开启服务器
    virtual public void StartOnline(string host, int port)
    {
        //定时器的配置
    ...
    virtual public void ProcessData(Conn conn)
    {
        ...
        Debug.Log("接收到了 " + conn.bufCount + " 个字节");
    ...
    virtual public void Send(Conn conn, ProtocolBase pro)
    {
        ...
        //将长度和信息粘合在一起
        byte[] sendbuff = length.Concat(bytes).ToArray();
        Debug.Log("发送的字节长度 " + sendbuff.Length);
        try
        {
            conn.socket.BeginSend(sendbuff, 0, sendbuff.Length, SocketFlags.None, null, null);
        }
        ...
        {
            Debug.Log("[发送消息 ]" + conn.GetAdress() + " : " + e.Message);
        }
    }
    ...
    virtual public void HeartBeat()
    {
        ...
        for (int i = 0; i < conns.Length; i++)
        {
            ...
            {
                Debug.Log("[心跳引起断开连接]" + conns[i].GetAdress() + "  该连接为" + i + "个池子");
                //加锁防止多线程占线关闭此套接字
                lock (conns[i])
                {
                    Debug.Log("关闭 " + i + "套接字    ");
